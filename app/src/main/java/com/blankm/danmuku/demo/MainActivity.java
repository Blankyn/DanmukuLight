...
//
        ImageView simpleDraweeView = findViewById(R.id.sdv_cover);

        Glide.with(this)
                .load("https://ww2.sinaimg.cn/large/610dc034jw1fa42ktmjh4j20u011hn8g.jpg")
...
//                .load(url);
    ...
    private void addDanmaku(DanmakuEntity danmakuEntity) {
        if (mDanMuHelper != null) {
            mDanMuHelper.addDanMu(danmakuEntity, true, new OnDanMuTouchCallBackListener() {
                @Override
    ...
    private void addRoomDanmaku(DanmakuEntity danmakuEntity) {
        if (mDanMuHelper != null) {
            mDanMuHelper.addDanMu(danmakuEntity, false, new OnDanMuTouchCallBackListener() {
                @Override
                public void callBack(DanMuModel danMuView) {
